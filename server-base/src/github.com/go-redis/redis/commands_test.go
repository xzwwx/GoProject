...
var _ = Describe("Commands", func() {
	...
	Describe("keys", func() {
		...
		It("should RandomKey", func() {
			...
			Expect(randomKey.Err()).To(Equal(Nil))
		...
		It("should Sort", func() {
			...
			els, err := client.Sort(ctx, "list", &Sort{
				Offset: 0,
				Count:  2,
				Order:  "ASC",
			}).Result()
		...
		It("should Sort and Get", func() {
			...
			{
				els, err := client.SortInterfaces(ctx, "list", &Sort{
					Get: []string{"object_*"},
				}).Result()
				Expect(err).NotTo(HaveOccurred())
		...
		It("should Sort and Store", func() {
			...
			n, err := client.SortStore(ctx, "list", "list2", &Sort{
				Offset: 0,
				Count:  2,
				Order:  "ASC",
			}).Result()
	...
	Describe("strings", func() {
		...
		It("should BitCount", func() {
			...
			bitCount = client.BitCount(ctx, "key", &BitCount{
				Start: 0,
				End:   0,
			})
			Expect(bitCount.Err()).NotTo(HaveOccurred())
		...
		It("should Set with keepttl", func() {
			...
			// set with keepttl
			set = client.Set(ctx, "key", "hello1", KeepTTL)
			Expect(set.Err()).NotTo(HaveOccurred())
			Expect(set.Val()).To(Equal("OK"))
		...
		It("should SetNX with keepttl", func() {
			isSet, err := client.SetNX(ctx, "key", "hello1", KeepTTL).Result()
			Expect(err).NotTo(HaveOccurred())
			Expect(isSet).To(Equal(true))
		...
		It("should SetXX with keepttl", func() {
			...
			isSet, err = client.SetXX(ctx, "key", "hello3", KeepTTL).Result()
			Expect(err).NotTo(HaveOccurred())
			Expect(isSet).To(Equal(true))
	...
	Describe("lists", func() {
		...
		It("should LIndex", func() {
			...
			Expect(lIndex.Err()).To(Equal(Nil))
		...
		It("should LPos", func() {
			...
			lPos := client.LPos(ctx, "list", "b", LPosArgs{})
			Expect(lPos.Err()).NotTo(HaveOccurred())
			Expect(lPos.Val()).To(Equal(int64(1)))

			lPos = client.LPos(ctx, "list", "b", LPosArgs{Rank: 2})
			Expect(lPos.Err()).NotTo(HaveOccurred())
			...
			lPos = client.LPos(ctx, "list", "b", LPosArgs{Rank: 2, MaxLen: 1})
			Expect(lPos.Err()).To(Equal(Nil))

			lPos = client.LPos(ctx, "list", "z", LPosArgs{})
			Expect(lPos.Err()).To(Equal(Nil))
		})
		...
		It("should LPosCount", func() {
			...
			lPos := client.LPosCount(ctx, "list", "b", 2, LPosArgs{})
			Expect(lPos.Err()).NotTo(HaveOccurred())
			Expect(lPos.Val()).To(Equal([]int64{1, 3}))

			lPos = client.LPosCount(ctx, "list", "b", 2, LPosArgs{Rank: 2})
			Expect(lPos.Err()).NotTo(HaveOccurred())
			Expect(lPos.Val()).To(Equal([]int64{3}))

			lPos = client.LPosCount(ctx, "list", "b", 1, LPosArgs{Rank: 1, MaxLen: 1})
			Expect(lPos.Err()).NotTo(HaveOccurred())
			Expect(lPos.Val()).To(Equal([]int64{}))

			lPos = client.LPosCount(ctx, "list", "b", 1, LPosArgs{Rank: 1, MaxLen: 0})
			Expect(lPos.Err()).NotTo(HaveOccurred())
			Expect(lPos.Val()).To(Equal([]int64{1}))
		})
	...
	Describe("sorted sets", func() {
		...
		It("should ZAdd", func() {
			added, err := client.ZAdd(ctx, "zset", &Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZAddArgs", func() {
			...
			added, err := client.ZAddArgs(ctx, "zset", ZAddArgs{
				GT:      true,
		...
		It("should ZAddNX", func() {
			added, err := client.ZAddNX(ctx, "zset", &Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZAddXX", func() {
			added, err := client.ZAddXX(ctx, "zset", &Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZAddCh", func() {
			changed, err := client.ZAddCh(ctx, "zset", &Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZAddNXCh", func() {
			changed, err := client.ZAddNXCh(ctx, "zset", &Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZAddXXCh", func() {
			changed, err := client.ZAddXXCh(ctx, "zset", &Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZIncr", func() {
			score, err := client.ZIncr(ctx, "zset", &Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZIncrNX", func() {
			score, err := client.ZIncrNX(ctx, "zset", &Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZIncrXX", func() {
			score, err := client.ZIncrXX(ctx, "zset", &Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).To(Equal(Nil))
		...
		It("should ZInterStore", func() {
			...
			err = client.ZAdd(ctx, "zset2", &Z{Score: 1, Member: "one"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset2", &Z{Score: 2, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset3", &Z{Score: 3, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())

			n, err := client.ZInterStore(ctx, "out", &ZStore{
		...
		It("should ZMScore", func() {
			...
			err := client.ZAdd(ctx, "zset", &Z{Score: 1, Member: "one"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset", &Z{Score: 2, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset", &Z{Score: 3, Member: "three"}).Err()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZRangeArgs", func() {
			...
			zRange, err := client.ZRangeArgs(ctx, ZRangeArgs{
				Key:     "zset",
				Start:   1,
				Stop:    4,
				ByScore: true,
			...
			// withScores.
			zSlice, err := client.ZRangeArgsWithScores(ctx, ZRangeArgs{
				Key:     "zset",
				Start:   1,
				Stop:    4,
				ByScore: true,
		...
		It("should ZRangeByScore", func() {
			...
			zRangeByScore := client.ZRangeByScore(ctx, "zset", &ZRangeBy{
				Min: "-inf",
				Max: "+inf",
			})
		...
		It("should ZRangeByLex", func() {
			...
			zRangeByLex := client.ZRangeByLex(ctx, "zset", &ZRangeBy{
				Min: "-",
				Max: "+",
			})
		...
		It("should ZRangeByScoreWithScoresMap", func() {
			...
			vals, err := client.ZRangeByScoreWithScores(ctx, "zset", &ZRangeBy{
				Min: "-inf",
				Max: "+inf",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZRangeStore", func() {
			...
			rangeStore, err := client.ZRangeStore(ctx, "new-zset", ZRangeArgs{
				Key:     "zset",
				Start:   1,
				Stop:    4,
				ByScore: true,
		...
		It("should ZRevRangeByLex", func() {
			err := client.ZAdd(ctx, "zset", &Z{Score: 0, Member: "a"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset", &Z{Score: 0, Member: "b"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset", &Z{Score: 0, Member: "c"}).Err()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZRevRank", func() {
			...
			Expect(zRevRank.Err()).To(Equal(Nil))
			Expect(zRevRank.Val()).To(Equal(int64(0)))
		})

		It("should ZScore", func() {
			zAdd := client.ZAdd(ctx, "zset", &Z{Score: 1.001, Member: "one"})
			Expect(zAdd.Err()).NotTo(HaveOccurred())
		...
		It("should ZUnion", func() {
			err := client.ZAddArgs(ctx, "zset1", ZAddArgs{
				Members: []Z{
					{Score: 1, Member: "one"},
					{Score: 2, Member: "two"},
				},
			...
			err = client.ZAddArgs(ctx, "zset2", ZAddArgs{
				Members: []Z{
					{Score: 1, Member: "one"},
					{Score: 2, Member: "two"},
					{Score: 3, Member: "three"},
			...
			union, err := client.ZUnion(ctx, ZStore{
				Keys:      []string{"zset1", "zset2"},
				Weights:   []float64{2, 3},
				Aggregate: "sum",
			}).Result()
			...
			unionScores, err := client.ZUnionWithScores(ctx, ZStore{
				Keys:      []string{"zset1", "zset2"},
				Weights:   []float64{2, 3},
				Aggregate: "sum",
			}).Result()
		...
		It("should ZUnionStore", func() {
			err := client.ZAdd(ctx, "zset1", &Z{Score: 1, Member: "one"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset1", &Z{Score: 2, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())
			...
			err = client.ZAdd(ctx, "zset2", &Z{Score: 2, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset2", &Z{Score: 3, Member: "three"}).Err()
			Expect(err).NotTo(HaveOccurred())

			n, err := client.ZUnionStore(ctx, "out", &ZStore{
				Keys:    []string{"zset1", "zset2"},
				Weights: []float64{2, 3},
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZDiff", func() {
			...
			err = client.ZAdd(ctx, "zset1", &Z{Score: 2, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset1", &Z{Score: 3, Member: "three"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd(ctx, "zset2", &Z{Score: 1, Member: "one"}).Err()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZInter", func() {
			...
			v, err := client.ZInter(ctx, &ZStore{
				Keys: []string{"zset1", "zset2"},
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZInterWithScores", func() {
			...
			v, err := client.ZInterWithScores(ctx, &ZStore{
				Keys:      []string{"zset1", "zset2"},
				Weights:   []float64{2, 3},
				Aggregate: "Max",
			}).Result()
	...
	Describe("streams", func() {
		BeforeEach(func() {
			id, err := client.XAdd(ctx, &XAddArgs{
				Stream: "stream",
				ID:     "1-0",
		...
		It("should XRead", func() {
			res, err := client.XRead(ctx, &XReadArgs{
			...
			_, err = client.XRead(ctx, &XReadArgs{
		...
		Describe("group", func() {
			BeforeEach(func() {
				...
				res, err := client.XReadGroup(ctx, &XReadGroupArgs{
					Group:    "group",
					Consumer: "consumer",
			...
			It("should XClaim", func() {
				msgs, err := client.XClaim(ctx, &XClaimArgs{
					Stream:   "stream",
					Group:    "group",
					Consumer: "consumer",
				...
				ids, err := client.XClaimJustID(ctx, &XClaimArgs{
					Stream:   "stream",
					Group:    "group",
					Consumer: "consumer",
					Messages: []string{"1-0", "2-0", "3-0"},
	...
	Describe("Geo add and radius search", func() {
		...
		It("should search geo radius", func() {
			res, err := client.GeoRadius(ctx, "Sicily", 15, 37, &GeoRadiusQuery{
				Radius: 200,
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should geo radius and store the result", func() {
			n, err := client.GeoRadiusStore(ctx, "Sicily", 15, 37, &GeoRadiusQuery{
				Radius: 200,
				Store:  "result",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should search geo radius by member with options", func() {
			res, err := client.GeoRadiusByMember(ctx, "Sicily", "Catania", &GeoRadiusQuery{
				Radius:      200,
				Unit:        "km",
				WithGeoHash: true,
				WithCoord:   true,
		...
		It("should search geo radius with no results", func() {
			res, err := client.GeoRadius(ctx, "Sicily", 99, 37, &GeoRadiusQuery{
				Radius:      200,
				Unit:        "km",
				WithGeoHash: true,
				WithCoord:   true,
