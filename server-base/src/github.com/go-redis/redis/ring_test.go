...
var _ = Describe("Redis Ring", func() {
	...
	BeforeEach(func() {
		...
		err := ring.ForEachShard(ctx, func(ctx context.Context, cl *Client) error {
	...
	Describe("pipeline", func() {
		It("distributes keys", func() {
			...
			for _, cmd := range cmds {
				...
				Expect(cmd.(*StatusCmd).Val()).To(Equal("OK"))
			}
		...
		It("is consistent with ring", func() {
			...
			_, err := ring.Pipelined(ctx, func(pipe Pipeliner) error {
	...
	Describe("new client callback", func() {
		It("can be initialized with a new client callback", func() {
			...
			opts.NewClient = func(name string, opt *Options) *Client {
	...
	Describe("Process hook", func() {
		...
		It("supports Process hook", func() {
			...
			ring.ForEachShard(ctx, func(ctx context.Context, shard *Client) error {
				shard.AddHook(&hook{
					beforeProcess: func(ctx context.Context, cmd Cmder) (context.Context, error) {
						Expect(cmd.String()).To(Equal("ping: "))
		...
		It("supports TxPipeline hook", func() {
			...
			_, err = ring.TxPipelined(ctx, func(pipe Pipeliner) error {
				pipe.Ping(ctx)
				return nil
			})
			Expect(err).NotTo(HaveOccurred())
...
var _ = Describe("Ring watch", func() {
	...
	It("should Watch", func() {
		...
		incr = func(key string) error {
			err := ring.Watch(ctx, func(tx *Tx) error {
	...
	It("should discard", func() {
		err := ring.Watch(ctx, func(tx *Tx) error {
			cmds, err := tx.TxPipelined(ctx, func(pipe Pipeliner) error {
	...
	It("returns no error when there are no commands", func() {
		err := ring.Watch(ctx, func(tx *Tx) error {
			_, err := tx.TxPipelined(ctx, func(Pipeliner) error { return nil })
			return err
		}, "key")
		Expect(err).NotTo(HaveOccurred())
	...
	It("respects max size on multi", func() {
		...
		ring.ForEachShard(ctx, func(ctx context.Context, cl *Client) error {
			defer GinkgoRecover()
...
var _ = Describe("Ring Tx timeout", func() {
	...
	Context("read/write timeout", func() {
		BeforeEach(func() {
			...
			err := ring.ForEachShard(ctx, func(ctx context.Context, client *Client) error {
		...
		AfterEach(func() {
			_ = ring.ForEachShard(ctx, func(ctx context.Context, client *Client) error {
				defer GinkgoRecover()
