...
func ExampleParseURL() {
	opt, err := ParseURL("redis://:qwerty@localhost:6379/1")
	if err != nil {
		panic(err)
	}
...
func ExampleNewFailoverClient() {
	...
	rdb := NewFailoverClient(&FailoverOptions{
		MasterName:    "master",
...
func ExampleNewClusterClient_manualSetup() {
	...
	clusterSlots := func(ctx context.Context) ([]ClusterSlot, error) {
		slots := []ClusterSlot{
...
func ExampleClient_Pipelined() {
	var incr *IntCmd
	_, err := rdb.Pipelined(ctx, func(pipe Pipeliner) error {
...
func Example_customCommand() {
	Get := func(ctx context.Context, rdb *Client, key string) *StringCmd {
...
func ExampleNewUniversalClient_simple() {
	rdb := NewUniversalClient(&UniversalOptions{
		Addrs: []string{":6379"},
	})
	defer rdb.Close()
