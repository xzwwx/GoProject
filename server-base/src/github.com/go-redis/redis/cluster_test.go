...
func (s *clusterScenario) masters() []*Client {
	result := make([]*Client, 3)
...
func (s *clusterScenario) slaves() []*Client {
	result := make([]*Client, 3)
	for pos, port := range s.ports[3:] {
		result[pos] = s.clients[port]
	}
...
func (s *clusterScenario) newClusterClientUnstable(opt *ClusterOptions) *ClusterClient {
	opt.Addrs = s.addrs()
	return NewClusterClient(opt)
}

func (s *clusterScenario) newClusterClient(
	ctx context.Context, opt *ClusterOptions,
) *ClusterClient {
...
func assertSlotsEqual(slots, wanted []ClusterSlot) error {
outerLoop:
...
var _ = Describe("ClusterClient", func() {
	...
	assertClusterClient := func() {
		...
		It("distributes keys", func() {
			...
			client.ForEachMaster(ctx, func(ctx context.Context, master *Client) error {
				defer GinkgoRecover()
				Eventually(func() string {
		...
		It("distributes scripts when using Script Load", func() {
			...
			script := NewScript(`return 'Unique script'`)

			script.Load(ctx, client)

			client.ForEachShard(ctx, func(ctx context.Context, shard *Client) error {
				defer GinkgoRecover()
		...
		It("checks all shards when using Script Exists", func() {
			...
			script := NewScript(`return 'First script'`)
			lostScriptSrc := `return 'Lost script'`
			lostScript := NewScript(lostScriptSrc)
		...
		It("flushes scripts from all shards when using ScriptFlush", func() {
			script := NewScript(`return 'Unnecessary script'`)
			script.Load(ctx, client)
		...
		Describe("pipelining", func() {
			...
			assertPipeline := func() {
				...
				It("follows redirects", func() {
					...
					_ = client.ForEachShard(ctx, func(ctx context.Context, node *Client) error {
						defer GinkgoRecover()
						Eventually(func() int64 {
					...
					for i, key := range keys {
						get := cmds[i*2].(*StringCmd)
						Expect(get.Val()).To(Equal(key + "_value"))

						ttl := cmds[(i*2)+1].(*DurationCmd)
	...
	Describe("ClusterClient", func() {
		...
		AfterEach(func() {
			_ = client.ForEachMaster(ctx, func(ctx context.Context, master *Client) error {
				return master.FlushDB(ctx).Err()
			})
		...
		It("returns pool stats", func() {
			...
			Expect(stats).To(BeAssignableToTypeOf(&PoolStats{}))
		})
		...
		It("supports Process hook", func() {
			...
			err = client.ForEachShard(ctx, func(ctx context.Context, node *Client) error {
			...
			clusterHook := &hook{
				beforeProcess: func(ctx context.Context, cmd Cmder) (context.Context, error) {
					Expect(cmd.String()).To(Equal("ping: "))
					stack = append(stack, "cluster.BeforeProcess")
					return ctx, nil
				},
				afterProcess: func(ctx context.Context, cmd Cmder) error {
		...
		It("supports Pipeline hook", func() {
			...
			client.AddHook(&hook{
				beforeProcessPipeline: func(ctx context.Context, cmds []Cmder) (context.Context, error) {
					Expect(cmds).To(HaveLen(1))
					Expect(cmds[0].String()).To(Equal("ping: "))
					stack = append(stack, "cluster.BeforeProcessPipeline")
					return ctx, nil
				},
				afterProcessPipeline: func(ctx context.Context, cmds []Cmder) error {
					Expect(cmds).To(HaveLen(1))
			...
			_, err = client.Pipelined(ctx, func(pipe Pipeliner) error {
				pipe.Ping(ctx)
				return nil
			})
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("ClusterClient with RouteByLatency", func() {
		BeforeEach(func() {
			...
			err = client.ForEachSlave(ctx, func(ctx context.Context, slave *Client) error {
				Eventually(func() int64 {
...
var _ = Describe("ClusterClient timeout", func() {
	...
	Context("read/write timeout", func() {
		BeforeEach(func() {
			...
			err := client.ForEachShard(ctx, func(ctx context.Context, client *Client) error {
		...
		AfterEach(func() {
			_ = client.ForEachShard(ctx, func(ctx context.Context, client *Client) error {
				defer GinkgoRecover()
				Eventually(func() error {
					return client.Ping(ctx).Err()
