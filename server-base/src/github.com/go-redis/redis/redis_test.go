...
var _ Hook = redisHookError{}

func (redisHookError) BeforeProcess(ctx context.Context, cmd Cmder) (context.Context, error) {
	return ctx, nil
}

func (redisHookError) AfterProcess(ctx context.Context, cmd Cmder) error {
...
var _ = Describe("Client", func() {
	...
	It("should return pool stats", func() {
		Expect(client.PoolStats()).To(BeAssignableToTypeOf(&PoolStats{}))
	})

	It("should support custom dialers", func() {
		custom := NewClient(&Options{
			Network: "tcp",
			Addr:    redisAddr,
	...
	It("should select DB", func() {
		db2 := NewClient(&Options{
			Addr: redisAddr,
			DB:   2,
		})
		Expect(db2.FlushDB(ctx).Err()).NotTo(HaveOccurred())
		Expect(db2.Get(ctx, "db").Err()).To(Equal(Nil))
		...
		Expect(client.Get(ctx, "db").Err()).To(Equal(Nil))
	...
	It("should retry with backoff", func() {
		clientNoRetry := NewClient(&Options{
			Addr:       ":1234",
			MaxRetries: -1,
		})
		defer clientNoRetry.Close()

		clientRetry := NewClient(&Options{
			Addr:            ":1234",
			MaxRetries:      5,
