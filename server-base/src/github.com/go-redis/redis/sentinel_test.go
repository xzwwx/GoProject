...
var _ = Describe("Sentinel", func() {
	...
	var sentinel *SentinelClient

	BeforeEach(func() {
		client = NewFailoverClient(&FailoverOptions{
	...
	It("should facilitate failover", func() {
		...
		Eventually(func() []string {
			slavesAddr = GetSlavesAddrByName(ctx, sentinel, sentinelName)
			return slavesAddr
		}, "15s", "100ms").Should(HaveLen(2))
		Eventually(func() bool {
			sync := true
			for _, addr := range slavesAddr {
				slave := NewClient(&Options{
					Addr:       addr,
					MaxRetries: -1,
				})
...
var _ = Describe("NewFailoverClusterClient", func() {
	...
	BeforeEach(func() {
		client = NewFailoverClusterClient(&FailoverOptions{
			MasterName:    sentinelName,
			SentinelAddrs: sentinelAddrs,
